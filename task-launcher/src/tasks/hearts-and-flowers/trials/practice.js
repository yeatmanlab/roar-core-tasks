  ...
  const trial = {
    ...
    stimulus: () => {
      ...
      const audioPrompt =
        stimulusType === StimulusType.Heart
          ? incorrect
            ? feedbackAudio.IncorrectHeart
            : feedbackAudio.CorrectHeart
          : incorrect
          ? feedbackAudio.IncorrectFlower
          : feedbackAudio.CorrectFlower;
      return audioPrompt;
    },
    prompt: () => {
      const stimulusType = store.session.get('stimulus');
    ...
    keyboard_choices: () => {
      ...
      if (store.session.get('correct') === false) {
        ...
        return validAnswerPosition === 0 ? InputKey.ArrowLeft : InputKey.ArrowRight;
      } else {
